...
fetchValidatorByPubkey(pubkey).then(balance => {
  if (balance !== null) {
    ...
    const writeMetricsToFile = async () => {
      ...
      fs.writeFileSync('/var/lib/prometheus/node-exporter/dill_balance.prom', metrics);
      console.log('Metrics written to file: /var/lib/prometheus/node-exporter/dill_balance.prom');
    };
